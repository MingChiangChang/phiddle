...
from crystalphase import CrystalPhase
from peak_profile import FixedPseudoVoigt
...
class labeler():
    ...
    def read_csv(self, csv):
        with open(csv, 'r') as f:
            t = f.read()
        self.phases = self.create_phases(t, .2, FixedPseudoVoigt(0.1))
    ...
    def create_phases(self, csv, width_init, peakprofile):
        sticks_data = csv.split('#')[:-1]

        phases = []
        for s in sticks_data:
            lat_info = s.split()[0].split(',')
            _id, phase_name, _, a, b, c, α, β, γ = lat_info 
            _id = int(_id)
            a, b, c, α, β, γ = list(map(float, [a, b, c, α, β, γ]))

            cl = get_crystal(a, b, c, α, β, γ)

            peaks = np.zeros((len(s.split())-1, 5))

            for idx, line in enumerate(s.split()[1:]):
                h, k, l, q, I = line.split(',')
                h, k, l = int(h), int(k), int(l)
                q, I = float(q), float(I)
                peaks[idx, :] = np.array([h, k, l, q, I]) 
            norm_constant = np.max(peaks[:,-1])
            peaks[:,-1] /= norm_constant

            phases.append(CrystalPhase(cl, cl, phase_name, peaks, peakprofile, norm_constant, _id=_id))
...
if __name__ == "__main__":
    ...
    l = labeler()
    csv_path = "/Users/ming/Desktop/Code/SARA.jl/BiTiO/cifs/sticks_tio2.csv"
    l.read_csv(csv_path)
    print(l.phases)
